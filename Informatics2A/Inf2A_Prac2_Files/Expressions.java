...
class MH_Exp_Impl implements MH_EXP {
    ...
    MH_Exp_Impl (MH_EXP left, char infixOp, MH_EXP right) {
	this.kind = 4 ;
	this.firstChild = left ;
	this.secondChild = right ;
	...
	if (exp.getLabel().equals("#Exp5")) {
		...
		// build '=' or '<' infix expression
		return new MH_Exp_Impl (head, i, other) ;
	    }
	...
	if (ops4.getRhs() == MH_Parser.epsilon) 
	    return new Stack() ;
	else {
	    MH_EXP exp = convertExp (ops4.getChildren()[0]) ;
	...
	if (ops3.getRhs() == MH_Parser.epsilon) 
	    ...
	    MH_EXP exp = convertExp (ops3.getChildren()[1]) ;
